...
function filterAgeGender(studentObj) {
    const lowerAgeLimit = 13;
    const upperAgeLimit = 18;
    const gender ='female';
    let ans = [];
  
    studentObj.forEach(studentData => {
      if ((studentData.age >= lowerAgeLimit && studentData.age <= upperAgeLimit)&&(studentData.gender==gender)){
  ...
  console.log(filterAgeGender(studentCopy))
